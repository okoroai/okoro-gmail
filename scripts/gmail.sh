#!/usr/bin/env bash
# gmail.sh — Gmail API via Okoro proxy with token session caching.
#
# Usage:
#   ./gmail.sh --endpoint <path> --intent <text> [--method GET] [--scope read] [--payload <json>]
#
# Required env:
...
if [[ -z "${OKORO_SERVICE_TOKEN:-}" ]]; then
  ...
  echo "" >&2
  echo "  This skill requires an Okoro service token to authenticate with Gmail." >&2
...
TOKEN_CACHE="${TMPDIR:-/tmp}/okoro_gmail_${_token_prefix}_${SCOPE}.tok"
...
_fetch_token() {
  ...
  local body
  body=$(jq -n     --arg provider "gmail"     --arg scope    "$SCOPE"     --arg intent   "$INTENT"     '{provider: $provider, scope: $scope, intent: $intent}')
  [[ -n "$refresh_token" ]] &&     body=$(printf '%s' "$body" | jq --arg t "$refresh_token" '. + {refresh_token: $t}')

  local response token exp
  response=$(curl -sf -X POST "https://okoro.ai/t/tokens"     -H "Authorization: Bearer ${OKORO_SERVICE_TOKEN}"     -H "Content-Type: application/json"     -d "$body")
